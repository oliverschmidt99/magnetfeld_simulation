...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function enableImagePanZoom(wrapperId, imageElement, transformState) {
        ...
        wrapper.addEventListener("mousedown", (e) => {
            e.preventDefault();
            isPanning = true;
            startPos = { x: e.clientX - transformState.translateX, y: e.clientY - transformState.translateY };
        ...
        const stopPan = () => { isPanning = false; wrapper.style.cursor = "grab"; };
    ...
    fetch("/api/analysis/runs")
        ...
        .then((runs) => {
            ...
            const lastSelection1 = JSON.parse(localStorage.getItem('resultsSelection1')) || {};
            const lastSelection2 = JSON.parse(localStorage.getItem('resultsSelection2')) || {};
            
            if(lastSelection1.run && runs[lastSelection1.run]) document.getElementById("run-selector-1").value = lastSelection1.run;
            if(lastSelection2.run && runs[lastSelection2.run]) document.getElementById("run-selector-2").value = lastSelection2.run;
    ...
    function populateRunSelectors(runs) {
        ...
        [selector1, selector2].forEach(selector => {
            ...
            runs.forEach((run, index) => {
                ...
                const name = `${date.slice(0, 4)}-${date.slice(4, 6)}-${date.slice(6, 8)} ${time.slice(0, 2)}:${time.slice(2, 4)}`;
    ...
    function setupSide(id) {
        ...
        const previewContainer = document.getElementById(`results-preview-container-${id}`);
        ...
        const densityLoading = document.getElementById(`density-plot-loading-${id}`);
        ...
        runSelector.addEventListener("change", (event) => handleRunChange(event.detail || {}));
        currentSelector.addEventListener("change", () => fetchPlotData());
        yAxisSelector.addEventListener("change", () => fetchPlotData());
        angleSlider.addEventListener("input", () => updatePlotByIndex(angleSlider.value));
        ...
        function handleRunChange(preselect = {}) {
            ...
            if (Array.from(currentSelector.options).some(o => o.value === preselectedCurrent)) {
        ...
        function fetchFullPreview(runFolder) {
            ...
            fetch(`/api/analysis/full_preview/${runFolder}`)
                ...
                .then(data => {
                    ...
                    renderInteractivePreview(`results-preview-container-${id}`, data.scenes, data.room, onStepClick);
                    equalizeCardHeights();
                })
                .catch(err => {
        ...
        function fetchPlotData(preselectYAxis = null) {
            ...
            if (!runIndex || !currentGroup || !currentPositionGroup) { return; }
            
            const selectedRun = simulationRuns[runIndex];
            const selectedConductors = Array.from(conductorSelector.querySelectorAll("input:checked")).map(cb => cb.value);
            const queryParams = new URLSearchParams({ run_folder: selectedRun.name, pos_group: currentPositionGroup, current_group: currentGroup, y_axis: desiredYAxis });
            ...
            fetch(`/api/analysis/plot?${queryParams.toString()}`)
                .then(res => res.ok ? res.json() : Promise.reject(new Error('Daten konnten nicht geladen werden')))
                .then(data => {
                    ...
                    data.columns.forEach(col => yAxisSelector.add(new Option(col.name, col.value)));
                    
                    if (Array.from(yAxisSelector.options).some(o => o.value === currentlySelectedYAxis)) {
                        yAxisSelector.value = currentlySelectedYAxis;
                    } else if (Array.from(yAxisSelector.options).some(o => o.value === desiredYAxis)) {
                    ...
                    if (id === 1 && conductorSelector.innerHTML === "") {
                        ...
                        data.conductors.forEach(c => {
                            ...
                            checkbox.type = "checkbox"; checkbox.value = c; checkbox.checked = true;
        ...
        function fetchFemmPlots() {
            ...
            stopAnimation();
            densityPlotList = [];
            angleSlider.disabled = true; angleSlider.value = 0; angleSlider.max = 0;
            
            femmContainer.classList.remove("initially-hidden");
            densityImg.classList.add("initially-hidden");
            densityLoading.style.display = "block"; densityLoading.textContent = "Lade Plot...";

            const queryParams = new URLSearchParams({ run_folder: selectedRun.name, pos_group: currentPositionGroup, current_group: currentGroup });
        ...
        function updateChart(data) {
            ...
            charts[id] = new Chart(ctx, { /* ... chart options ... */ });
        ...
        enableImagePanZoom(`density-plot-wrapper-${id}`, densityImg, imageTransform);
